...
export const verifyKey = async key => {
  try {
    ...
    // Assigns the result of verifyKey to decodedKey
    const decodedKey = jwt.verify(key, signingKey);
    console.log("DECODED_LEY: ", decodedKey);
    ...
    // Catches the expired token error and removes the session from storage, Returns false
    Sessions.where({ sessionId: key }).findOneAndDelete(() => {
      return false;
    });
  }
};
